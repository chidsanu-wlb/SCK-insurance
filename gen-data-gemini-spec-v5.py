...
def generate_thai_id():
    """สร้างเลขบัตรประชาชน 13 หลักตาม Checksum Modulo 11"""
...
def format_thai_date(dt):
    """แปลง datetime เป็น รูปแบบ '15 สิงหาคม 2569'"""
    day = dt.day
...
def calculate_age(birth_date, ref_date):
    """คำนวณอายุเป็น ปี และ วัน ณ วันที่อ้างอิง"""
    years = ref_date.year - birth_date.year
    
    # คำนวณวันเกิดล่าสุดก่อนหน้าหรือตรงกับ ref_date
    try:
        last_birthday = datetime(ref_date.year, birth_date.month, birth_date.day)
    except ValueError: # กรณีเกิด 29 ก.พ. ในปีที่ไม่ใช่ leap year
...
def get_random_birthdate_for_adult(status_type, advance_days):
    """สุ่มวันเกิดสำหรับผู้ใหญ่ (ผู้ถือกรมธรรม์/คู่สมรส)"""
    if "อายุเกิน" in status_type:
        # 65 ปี + advance_days + 1 วัน ถึง 66 ปี ณ RENEWAL_DATE
        min_days = int(65 * 365.25) + advance_days + 1
        max_days = int(66 * 365.25)
    else: # ปกติ หรือ เสียชีวิต
        # 20 - 30 ปี ณ RENEWAL_DATE
...
def get_random_birthdate_for_child(status_type, advance_days):
    """สุ่มวันเกิดสำหรับบุตร"""
    if "อายุเกิน" in status_type:
        # 15 ปี + advance_days + 1 วัน ถึง 16 ปี ณ RENEWAL_DATE
        min_days = int(15 * 365.25) + advance_days + 1
        max_days = int(16 * 365.25)
    else: # ปกติ หรือ เสียชีวิต
        # 1 - 13 ปี ณ RENEWAL_DATE
...
def create_person_info_str(first_name, last_name, birth_date):
    """สร้างก้อนข้อความแสดงข้อมูลบุคคล"""
...
if __name__ == "__main__":
    process_mock_data("SCK insurance - Sheet2.csv", "SCK_insurance_Output_Gemini.csv")